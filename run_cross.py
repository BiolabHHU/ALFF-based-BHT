...
def prepare_data(index, train_data_name, test_data_name):

    # get functional connections and their labels by matlab code
    train_h0_data, train_h0_label, train_h1_data, train_h1_label, test_h0_label, test_h1_label, rank_h0, rank_h1 = eng.svm_two_suppose_ALFF_cross_double(index, train_data_name, test_data_name, nargout=8)
...
def train():
    ...
    for i in range(dict_data[test_name_of_data]):

        train_h0_data, train_h0_label, train_h1_data, train_h1_label, num_h0, num_h1, test_h0_label, test_h1_label, \
        rank_h0, rank_h1 = prepare_data(index=i + 1, train_data_name=train_name_of_data, test_data_name=test_name_of_data)
        ...
        print('\n current loop:' + str(i+1) + ' / ' + str(dict_data[test_name_of_data]) + '-------------')
    ...
    tyb2 = '1 Accuracy: {}%'
    print(tyb2.format(100 * k / dict_data[test_name_of_data]))
    tyb3 = '2 Sensitivity: {}%'
    ...
    print('train_data:{}, test_data:{}'.format(train_name_of_data, test_name_of_data))
    # pred_rank = np.array(pred_rank)
    # scio.savemat('./results/rank/{}-{}-rank.mat'.format(name_of_data, j_out), {'rank': pred_rank})
    scio.savemat('./save_model/{}/pre_lab_{}_{}.mat'.format(train_name_of_data, test_name_of_data, j_out), {'pre_lab': pred_tyb})
    results_txt = str(AD2AD) + '\t' + str(AD2HC) + '\t' + str(HC2HC) + '\t' + str(HC2AD) + '\t' + str(
        100 * k / dict_data[test_name_of_data]) + '\t' + str(100 * AD2AD / (AD2AD + AD2HC)) + '\t' + str(
        100 * HC2HC / (HC2AD + HC2HC)) + '\t' + str(100 * AD2AD / (AD2AD + HC2AD)) + '\t' + str(
        100 * 2 * sensitivity * precision / (sensitivity + precision)) + '\t' + str(AUC) + '\n'

    with open('./results/' + train_name_of_data + '.txt', "a+") as f:
        f.write(results_txt)
...
if __name__ == '__main__':
    ...
    EPOCH_list = {'NYU_alff_ds': 100, 'PU_alff_ds': 100, 'KKI_alff_ds': 150, 'NI_alff_ds': 35}
    ...
    for i_out in range(0, 1):   # select ADHD-200 datasets
        ...
        train_name_of_data = name_list[i_out] #实际上为训练数据集
        test_data_name_ind = [1, 2, 3]
        for t_out in test_data_name_ind:
            test_name_of_data = name_list[t_out]
            with open('./results/' + train_name_of_data + '.txt', "a+") as f:
                f.write('test_data_name: {}\n'.format(test_name_of_data))
            num_row_feature = 25
            num_of_hidden = 10              # neural unit in auto-coding network
            num_of_hidden_classify = 8     # neural unit in classification network
            Batch_size = dict_data[train_name_of_data]
            EPOCH = EPOCH_list[train_name_of_data]

            for j_out in range(0, 10):
                encoder = encode()
                decoder = decode()
                classifier = classify()
