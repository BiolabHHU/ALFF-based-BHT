...
def judge2(y_h0_x, h0_label, y_h1_x, h1_label, num_h0, num_h1):
    ...
    # inter- and intra-class distance
    yh1_AD_avg = np.mean(yh1_AD, axis=(0,))  # h1 mean of ADHD
    yh1_HC_avg = np.mean(yh1_HC, axis=(0,))  # h1 HC
...
def train():
    ...
    print(name_of_data)
    scio.savemat('./save_model/{}/pre_lab_{}.mat'.format(name_of_data, j_out), {'pre_lab': pred_tyb})
...
if __name__ == '__main__':
    name_list = ['NYU', 'PU', 'KKI', 'NI']
    dict_data = {'NYU': 168, 'PU': 150, 'KKI': 68, 'NI': 48}
    EPOCH_list = {'NYU': 100, 'PU': 100, 'KKI': 150, 'NI': 35}
    for i_out in range(0, 3):   # select ADHD-200 datasets
